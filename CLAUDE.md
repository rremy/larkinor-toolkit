...
- **The game's total width is 791px** (its widest element is the top banner; a right-hand sidebar runs `653–791`). Everything past that is empty page on a desktop window, which is where the minimised database overlay docks — `src/desktop/boot.ts` measures it at boot and publishes it as `--lc-game-right`. The usable docked width is therefore `window width − 791`: generous on a wide monitor (~720px at 1513), cramped below about 1200.
